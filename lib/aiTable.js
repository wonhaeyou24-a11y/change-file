...
async function callGemini(system, turns, apiKey, model) {
  ...
  const generationConfig = { temperature: 0, maxOutputTokens: 32000, responseMimeType: "application/json" };
...
async function aiBuildTable({ provider, apiKey, model, template, dataTSV, instructions }) {
  ...
  const useModel = conf.models.includes(model) ? model : conf.default;
...
module.exports = { aiBuildTable, PROVIDERS };
