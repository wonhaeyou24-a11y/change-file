...
시설물(절토사면/옹벽 등) "점검진단실적" 원본 엑셀을 보고서용 "이력현황" 표(Word .docx + Excel
.xlsx)로 변환하는 웹앱입니다. `inspection-history-table` 스킬의 표 양식(절토사면형/옹벽형)을
그대로 옮겼습니다.

**AI를 쓰지 않습니다.** 문장이 셀 너비 때문에 줄바꿈으로 끊긴 걸 다시 이어붙이는 것도 규칙 기반
로직(`lib/ruleClean.js`)으로 처리합니다 — 그래서 별도 API 키나 외부 호출 없이 동작합니다.
다만 규칙만으로는 줄바꿈 위치를 100% 완벽히 판단할 수 없어서(특히 불릿 문자가 애매하게 찍힌
경우) 가끔 한 문장이 두 항목으로 나뉘어 보일 수 있습니다 — 이 경우도 **글자·단어는 항상 원문
그대로**이고 항목이 나뉘는 위치만 다를 수 있으니, 변환 후 표를 열어 필요하면 줄바꿈/띄어쓰기만
다듬어 쓰시면 됩니다.
